...
export default {
  ...
  render(h) {
    return h('div', {
      staticClass: 'flex no-wrap relative-position items-center',
      style: {minHeight: '35px', borderBottom: '1px solid var(--q-color-grey-4)'}
    }, [
      this.$slots.start,
    ...
    render_item(h, item) {
      ...
      let data = this.disable ? {'class': 'cursor-not-allowed', style: {opacity: 0.4}}
        : same ? {'class': 'text-primary '}
          : {
            'class': 'text-grey-5 pp-selectable-color-blue-4 cursor-pointer',
